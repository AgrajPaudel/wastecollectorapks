...
void take_address() async {
  ...
  for (int i = 0; i < querySnapshots!.docs.length; i++) {
    ...
    if (address == data['address'] && data['state'] != 'completed') {
      if (data['latitude'] != null && data['longitude'] != null) {
        if (data['state'] == null ||
            data['state'] == emailid ||
            data['state'] == 'uncollected') {
          //date addition left
          ...
          //date wala
          String date1;
          date1 = data['state'].toString();
          DateTime date = DateTime.parse(date1);
          DateTime today = DateTime.now();
          if (today.year != date.year ||
              today.month != date.month ||
              today.day != date.day) {
            print('Today is a different date than the given date.');
            print(data['latitude'].toString() +
                '        0            ' +
                data['longitude'].toString());
            values.add(LatLng(
          ...
          return GestureDetector(
            onTap: () async {
              ...
              if (assetaddress[i] == 'assets/marker1.png') {
                ...
                CollectionReference map_address =
                    FirebaseFirestore.instance.collection('addresses');
                map_address.doc(docid[i]).update({'state': null});
...
class Inside_collection extends StatefulWidget {
  const Inside_collection({Key? key}) : super(key: key);

  @override
  State<Inside_collection> createState() => InsideCollectionviewstate();
}

class InsideCollectionviewstate extends State<Inside_collection> {
